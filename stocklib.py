...
def pick_stocks(df, cash_to_invest, n_stocks, mmt_var, top_by_mmt):
    ...
    stocks_by_mmt_pb = top_by_mmt.sort_values(
        'price_to_book_ratio').head(n_stocks).copy()
    stocks_by_mmt_pb['shares'] = cash_for_each_stock / \
        stocks_by_mmt_pb['stock_price']
    stocks_by_mmt_pb_simple = stocks_by_mmt_pb[['stock', 'shares', 'stock_price',
                                                mmt_var, 'price_to_book_ratio',
                                               'book_value_per_share']]
    # Change momentum to percentage
    stocks_by_mmt_pb_simple[mmt_var + '_pct'] = \
        (stocks_by_mmt_pb_simple[mmt_var] * 100).round()
